/**
 * Reads pages 1–5 of PDFs in studying/embedded-iot (and aliases) and writes
 * src/data/embeddedIotStudyBookMetadata.json.
 * Run: node scripts/extract-embedded-iot-book-metadata.mjs
 */
import fs from 'fs';
...
/** Same order as src/lib/embeddedIotStudyDirs.ts */
const EMBEDDED_IOT_FOLDER_NAMES = [
  'embedded-iot',
  'embedded iot',
  'Embedded  IoT',
  'Embedded IoT',
...
const outPath = path.join(root, 'src', 'data', 'embeddedIotStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const iotBoost =
    /\b(embedded\s+systems?|internet\s+of\s+things|\bIoT\b|microcontroller|\bMCU\b|Arduino|ESP32|ESP8266|Raspberry\s+Pi|\bRTOS\b|FreeRTOS|Zephyr|MQTT|CoAP|sensor|actuator|firmware|bare\s+metal|GPIO|\bSPI\b|\bI2C\b|\bUART\b|Bluetooth\s+Low\s+Energy|\bBLE\b|LoRa|edge\s+computing|industrial\s+IoT|smart\s+device|wearable|IoT\s+(cloud|gateway|protocol))\b/i;
  ...
  for (const line of head) {
    ...
    if (iotBoost.test(line)) score += 38;
    if (/introduction|handbook|manual|guide|edition|course|tutorial|fundamentals|programming|design|development/i.test(line))
      score += 12;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && iotBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Embedded / IoT study material (PDF).';
...
async function main() {
  ...
  const bookDirs = EMBEDDED_IOT_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'embedded-iot');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Embedded / IoT study material (PDF).' };
      }
    }
  }
