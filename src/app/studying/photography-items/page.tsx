...
import photographyStudyBookMetadata from '@/data/photographyStudyBookMetadata.json';
...
type PhotographyBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const photographyMeta = photographyStudyBookMetadata as Record<string, PhotographyBookMeta>;
...
function getPhotographyBookItems(): StudyItem[] {
  const bookDirs = resolvePhotographyStudyBookDirs();
  if (!bookDirs.length) return [];

  const imageDirCandidates = [
    path.join(process.cwd(), 'image', 'Photography'),
    path.join(process.cwd(), 'image', 'photography'),
    path.join(process.cwd(), 'image', 'Photo'),
    path.join(process.cwd(), 'image', 'photo'),
  ];
  const photographyImageDir = imageDirCandidates.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = photographyImageDir
    ? fs
        .readdirSync(photographyImageDir, { withFileTypes: true })
  ...
  return pdfEntries
    .map((f) => {
      ...
      if (matchedImage && photographyImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(photographyImageDir), matchedImage);
      } else if (remainingImages.length && photographyImageDir) {
        imageHref = toStudyImageHref(path.basename(photographyImageDir), remainingImages.shift()!);
...
export default function StudyingPhotographyItemsPage() {
  const items = getPhotographyBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">
          Photography study books
        </h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/photography</strong> or{' '}
          <strong className="font-semibold text-ink">studying/Photography</strong>. Cover images:{' '}
          <strong className="font-semibold text-ink">image/Photography</strong>. Metadata:{' '}
          <strong className="font-semibold text-ink">photographyStudyBookMetadata.json</strong> — run{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run photography-books:metadata
          </code>
          .
        </p>
        ...
        {items.length === 0 ? (
          <p className="mt-8 rounded-xl border border-parchment-300/80 bg-parchment-50/90 px-4 py-6 font-sans text-sm leading-relaxed text-ink-muted">
            ...
            <code className="font-mono text-ink">studying/photography</code> or{' '}
            <code className="font-mono text-ink">studying/Photography</code>, then run{' '}
            <code className="font-mono text-ink">npm run photography-books:metadata</code>.
          </p>
        ) : (
