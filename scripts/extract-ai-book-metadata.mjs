/**
 * Reads pages 1-2 of each PDF in studying/AI/AI and writes
 * src/data/aiStudyBookMetadata.json with inferred title + summary text.
 * Run: node scripts/extract-ai-book-metadata.mjs
 */
import fs from 'fs';
...
const booksDir = path.join(root, 'studying', 'AI', 'AI');
const outPath = path.join(root, 'src', 'data', 'aiStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  for (const line of lines.slice(0, 40)) {
    ...
    if (/artificial intelligence|machine learning|deep learning|neural|reinforcement|ai/i.test(line)) score += 28;
    if (/introduction|guide|handbook|basics|for dummies|student/i.test(line)) score += 10;
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'AI study material (PDF).';
...
async function main() {
  ...
  for (const name of files) {
    ...
    try {
      ...
      out[name] = { title: prettify(name), description: 'AI study material (PDF).' };
    }
  }
