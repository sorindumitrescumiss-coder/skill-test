...
import musicProductionStudyBookMetadata from '@/data/musicProductionStudyBookMetadata.json';
import { resolveMusicProductionStudyBookDirs } from '@/lib/musicProductionStudyDirs';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type MusicProductionBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const musicProductionMeta = musicProductionStudyBookMetadata as Record<string, MusicProductionBookMeta>;
...
function getMusicProductionBookItems(): StudyItem[] {
  const bookDirs = resolveMusicProductionStudyBookDirs();
  if (!bookDirs.length) return [];

  const imageDirCandidates = [
    path.join(process.cwd(), 'image', 'Music production'),
    path.join(process.cwd(), 'image', 'music-production'),
    path.join(process.cwd(), 'image', 'music production'),
    path.join(process.cwd(), 'image', 'Music Production'),
  ];
  const musicProductionImageDir = imageDirCandidates.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = musicProductionImageDir
    ? fs
        .readdirSync(musicProductionImageDir, { withFileTypes: true })
  ...
  return pdfEntries
    .map((f) => {
      ...
      if (matchedImage && musicProductionImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(musicProductionImageDir), matchedImage);
      } else if (remainingImages.length && musicProductionImageDir) {
        imageHref = toStudyImageHref(path.basename(musicProductionImageDir), remainingImages.shift()!);
      ...
      const meta = musicProductionMeta[f.name];
      const title = isReadableText(meta?.title) ? meta!.title : prettifyName(f.name);
      const description = isReadableText(meta?.description)
        ? meta!.description
        : 'Music production study material (PDF).';

      return {
        title,
        description,
        href: `/music-production-books/${encodeURIComponent(f.name)}`,
        imageHref,
      };
    })
...
export default function StudyingMusicProductionItemsPage() {
  const items = getMusicProductionBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">
          Music production study books
        </h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/music-production</strong> or{' '}
          <strong className="font-semibold text-ink">studying/Music production</strong>. Cover images:{' '}
          <strong className="font-semibold text-ink">image/Music production</strong>. Metadata:{' '}
          <strong className="font-semibold text-ink">musicProductionStudyBookMetadata.json</strong> — run{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run music-production-books:metadata
          </code>
          .
        </p>
        ...
        {items.length === 0 ? (
          <p className="mt-8 rounded-xl border border-parchment-300/80 bg-parchment-50/90 px-4 py-6 font-sans text-sm leading-relaxed text-ink-muted">
            ...
            <code className="font-mono text-ink">studying/music-production</code> or{' '}
            <code className="font-mono text-ink">studying/Music production</code>, then run{' '}
            <code className="font-mono text-ink">npm run music-production-books:metadata</code>.
          </p>
        ) : (
