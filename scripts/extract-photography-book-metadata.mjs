/**
 * Reads pages 1–5 of PDFs in studying/photography (and aliases) and writes
 * src/data/photographyStudyBookMetadata.json.
 * Run: node scripts/extract-photography-book-metadata.mjs
 */
import fs from 'fs';
...
const PHOTOGRAPHY_FOLDER_NAMES = ['photography', 'Photography', 'photo'];
const outPath = path.join(root, 'src', 'data', 'photographyStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const photographyBoost =
    /\b(photography|photo|camera|lens|exposure|aperture|shutter|iso|composition|lighting|portrait|landscape|macro|retouch|editing|raw|color\s+grading|white\s+balance|depth\s+of\s+field|histogram)\b/i;
  ...
  for (const line of head) {
    ...
    if (photographyBoost.test(line)) score += 38;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && photographyBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Photography study material (PDF).';
...
async function main() {
  ...
  const bookDirs = PHOTOGRAPHY_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'photography');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Photography study material (PDF).' };
      }
    }
  }
