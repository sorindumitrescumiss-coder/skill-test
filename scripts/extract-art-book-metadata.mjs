/**
 * Reads pages 1–5 of each PDF in studying/art and writes
 * src/data/artStudyBookMetadata.json with inferred title + summary text.
 * Run: node scripts/extract-art-book-metadata.mjs
 */
import fs from 'fs';
...
const booksDir = path.join(root, 'studying', 'art');
const outPath = path.join(root, 'src', 'data', 'artStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const artBoost =
    /\b(art|artist|artistic|painting|paint|illustrat|draw|sketch|design|color|colour|composition|figure|canvas|brush|palette|watercolor|watercolour|oil painting|acrylic|digital art|concept art|fine art|gallery|museum|sculpt|ceramic|printmak|typography|lettering|pixel art|vector|mixed media|collage|creativ|craft|aesthetic|visual culture|art history|renaissance|baroque|modern art|contemporary art)\b/i;
  ...
  for (const line of head) {
    ...
    if (artBoost.test(line)) score += 38;
    if (/introduction|handbook|manual|guide|edition|course|workshop|tutorial|basics|fundamentals|primer/i.test(line))
      score += 14;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && artBoost.test(line)) score += 24;
    if (joined.includes(line) && /admission|requirement|syllabus/i.test(line) && line.length < 80) score += 8;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Art study material (PDF).';
...
async function main() {
  ...
  for (const name of files) {
    ...
    try {
      ...
      out[name] = { title: prettify(name), description: 'Art study material (PDF).' };
    }
  }
