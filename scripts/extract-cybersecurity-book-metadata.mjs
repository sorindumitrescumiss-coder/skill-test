/**
 * Reads pages 1–5 of PDFs in studying/cybersecurity (and aliases) and writes
 * src/data/cybersecurityStudyBookMetadata.json.
 * Run: node scripts/extract-cybersecurity-book-metadata.mjs
 */
import fs from 'fs';
...
const CYBER_FOLDER_NAMES = ['cybersecurity', 'Cybersecurity', 'cyber-security', 'cyber security'];
const outPath = path.join(root, 'src', 'data', 'cybersecurityStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  for (const line of head) {
    ...
    if (secBoost.test(line)) score += 38;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && secBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Cybersecurity study material (PDF).';
...
async function main() {
  ...
  const bookDirs = CYBER_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'cybersecurity');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Cybersecurity study material (PDF).' };
      }
    }
  }
