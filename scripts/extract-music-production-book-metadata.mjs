/**
 * Reads pages 1–5 of PDFs in studying/music-production (and aliases) and writes
 * src/data/musicProductionStudyBookMetadata.json.
 * Run: node scripts/extract-music-production-book-metadata.mjs
 */
import fs from 'fs';
...
const MUSIC_PRODUCTION_FOLDER_NAMES = ['music-production', 'Music production', 'music production'];
const outPath = path.join(root, 'src', 'data', 'musicProductionStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const musicBoost =
    /\b(music\s+production|audio|sound\s+design|mixing|mastering|recording|studio|daw|ableton|logic\s+pro|fl\s+studio|pro\s+tools|midi|synth|arrangement|equalization|eq|compression|reverb|delay|sampling|electronic\s+music)\b/i;
  ...
  for (const line of head) {
    ...
    if (musicBoost.test(line)) score += 38;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && musicBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Music production study material (PDF).';
...
async function main() {
  ...
  const bookDirs = MUSIC_PRODUCTION_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'music-production');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Music production study material (PDF).' };
      }
    }
  }
