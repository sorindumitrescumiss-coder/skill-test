/**
 * Reads pages 1–5 of each PDF in studying/mobile-development and writes
 * src/data/mobileDevelopmentStudyBookMetadata.json with inferred title + summary text.
 * Run: node scripts/extract-mobile-development-book-metadata.mjs
 */
import fs from 'fs';
...
/** Same order as {@link src/lib/mobileDevelopmentStudyDirs.ts} — first folder wins on duplicate filenames. */
const MOBILE_STUDY_FOLDER_NAMES = ['mobile-development', 'mobile development', 'mobile'];
const outPath = path.join(root, 'src', 'data', 'mobileDevelopmentStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const mobileBoost =
    /\b(mobile\s+(app|application|development)|Android|iOS|Kotlin|Swift|SwiftUI|Jetpack|Flutter|React\s+Native|Xamarin|MAUI|Cordova|Capacitor|Ionic|smartphone|tablet|native\s+app|cross[\s-]platform|Play\s+Store|App\s+Store|mobile\s+UX|mobile\s+backend|push\s+notification|Firebase|Gradle|Xcode)\b/i;
  ...
  for (const line of head) {
    ...
    if (mobileBoost.test(line)) score += 38;
    if (/introduction|handbook|manual|guide|edition|course|workshop|tutorial|basics|fundamentals|primer|essential|programming/i.test(line))
      score += 14;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && mobileBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Mobile development study material (PDF).';
...
async function main() {
  ...
  const bookDirs = MOBILE_STUDY_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'mobile-development');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Mobile development study material (PDF).' };
      }
    }
  }
