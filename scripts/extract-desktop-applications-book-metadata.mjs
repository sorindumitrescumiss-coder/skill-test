/**
 * Reads pages 1–5 of PDFs in studying/desktop-applications (and aliases) and writes
 * src/data/desktopApplicationsStudyBookMetadata.json.
 * Run: node scripts/extract-desktop-applications-book-metadata.mjs
 */
import fs from 'fs';
...
/** Same order as src/lib/desktopApplicationsStudyDirs.ts */
const DESKTOP_STUDY_FOLDER_NAMES = ['desktop-applications', 'desktop application', 'desktop'];
const outPath = path.join(root, 'src', 'data', 'desktopApplicationsStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const desktopBoost =
    /\b(desktop\s+(app|application|software)|native\s+app|cross[\s-]platform\s+desktop|WinForms|\bWPF\b|UWP|Electron|Tauri|Qt|GTK|\.NET\s+(desktop|\bMAUI\b)|Windows\s+Forms|macOS\s+app|SwiftUI\s+mac|\bCocoa\b|X11|Linux\s+GUI|installer|MSI|ClickOnce|thick\s+client|rich\s+client|offline\s+first)\b/i;
  ...
  for (const line of head) {
    ...
    if (desktopBoost.test(line)) score += 38;
    if (/software\s+development|application|programming|handbook|manual|guide|edition|course|introduction|basics|fundamentals|computer/i.test(line))
      score += 12;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && desktopBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Desktop applications study material (PDF).';
...
async function main() {
  ...
  const bookDirs = DESKTOP_STUDY_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'desktop-applications');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Desktop applications study material (PDF).' };
      }
    }
  }
