...
import gameStudyBookMetadata from '@/data/gameStudyBookMetadata.json';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type GameBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const gameMeta = gameStudyBookMetadata as Record<string, GameBookMeta>;
...
function getGameBookItems(): StudyItem[] {
  const booksDir = path.join(process.cwd(), 'studying', 'game');
  if (!fs.existsSync(booksDir)) return [];

  const gameImageDirCandidates = [path.join(process.cwd(), 'image', 'game'), path.join(process.cwd(), 'image', 'Game')];
  const gameImageDir = gameImageDirCandidates.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = gameImageDir
    ? fs
        .readdirSync(gameImageDir, { withFileTypes: true })
  ...
  return files
    ...
    .map((f) => {
      ...
      if (matchedImage && gameImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(gameImageDir), matchedImage);
      } else if (remainingImages.length && gameImageDir) {
        const fallback = remainingImages.shift()!;
        imageHref = toStudyImageHref(path.basename(gameImageDir), fallback);
      ...
      return {
        title: isReadableText(gameMeta[f.name]?.title) ? gameMeta[f.name]!.title : prettifyName(f.name),
        description: isReadableText(gameMeta[f.name]?.description)
          ? gameMeta[f.name]!.description
          : 'Game study material (PDF).',
        href: `/game-books/${encodeURIComponent(f.name)}`,
        imageHref,
      };
    })
...
export default function StudyingGameItemsPage() {
  const items = getGameBookItems();
  ...
  return (
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">Game study books</h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          These books are loaded from <strong className="font-semibold text-ink">studying/game</strong>.
        </p>
