/**
 * Reads pages 1–5 of each PDF in studying/marketing and writes
 * src/data/marketingStudyBookMetadata.json with inferred title + summary text.
 * Run: node scripts/extract-marketing-book-metadata.mjs
 */
import fs from 'fs';
...
const booksDir = path.join(root, 'studying', 'marketing');
const outPath = path.join(root, 'src', 'data', 'marketingStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const marketingBoost =
    /\b(marketing|market\s+research|market\s+analysis|brand|branding|consumer|customer|advertis|digital\s+marketing|SEO|SEM|social\s+media|campaign|sales\s+strategy|public\s+relations|\bPR\b|CRM|analytics|segmentation|positioning|value\s+proposition|lead\s+generation|content\s+marketing|email\s+marketing|affiliate|growth|go-to-market|B2B|B2C|product\s+marketing|promotion|pricing|distribution|channel|Kotler|STP|SWOT|4\s*P|four\s+Ps)\b/i;
  ...
  for (const line of head) {
    ...
    if (marketingBoost.test(line)) score += 38;
    if (/introduction|handbook|manual|guide|edition|course|workshop|tutorial|basics|fundamentals|primer|management/i.test(line))
      score += 14;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && marketingBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Marketing study material (PDF).';
...
async function main() {
  ...
  for (const name of files) {
    ...
    try {
      ...
      out[name] = { title: prettify(name), description: 'Marketing study material (PDF).' };
    }
  }
