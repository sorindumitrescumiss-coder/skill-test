...
import artStudyBookMetadata from '@/data/artStudyBookMetadata.json';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type ArtBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const artMeta = artStudyBookMetadata as Record<string, ArtBookMeta>;
...
function getArtBookItems(): StudyItem[] {
  const booksDir = path.join(process.cwd(), 'studying', 'art');
  if (!fs.existsSync(booksDir)) return [];

  const artImageDirs = [
    path.join(process.cwd(), 'image', 'art'),
    path.join(process.cwd(), 'image', 'Art'),
  ];
  const artImageDir = artImageDirs.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = artImageDir
    ? fs
        .readdirSync(artImageDir, { withFileTypes: true })
  ...
  return files
    ...
    .map((f) => {
      ...
      if (matchedImage && artImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(artImageDir), matchedImage);
      } else if (remainingImages.length && artImageDir) {
        imageHref = toStudyImageHref(path.basename(artImageDir), remainingImages.shift()!);
      ...
      const meta = artMeta[f.name];
      const title = isReadableText(meta?.title) ? meta!.title : prettifyName(f.name);
      const description = isReadableText(meta?.description) ? meta!.description : 'Art study material (PDF).';

      return {
        title,
        description,
        href: `/art-books/${encodeURIComponent(f.name)}`,
        imageHref,
      };
    })
...
export default function StudyingArtItemsPage() {
  const items = getArtBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/art</strong>. Cover images can live in{' '}
          <strong className="font-semibold text-ink">image/art</strong> (match the PDF basename when possible).
          Titles and summaries are stored in{' '}
          <strong className="font-semibold text-ink">artStudyBookMetadata.json</strong> — refresh with{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run art-books:metadata
          </code>{' '}
