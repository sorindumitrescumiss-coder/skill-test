...
import productDesignStudyBookMetadata from '@/data/productDesignStudyBookMetadata.json';
import { resolveProductDesignStudyBookDirs } from '@/lib/productDesignStudyDirs';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type ProductDesignBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const productDesignMeta = productDesignStudyBookMetadata as Record<string, ProductDesignBookMeta>;
...
function getProductDesignBookItems(): StudyItem[] {
  const bookDirs = resolveProductDesignStudyBookDirs();
  if (!bookDirs.length) return [];

  const imageDirCandidates = [
    path.join(process.cwd(), 'image', 'Product design'),
    path.join(process.cwd(), 'image', 'product-design'),
    path.join(process.cwd(), 'image', 'product design'),
    path.join(process.cwd(), 'image', 'Product Design'),
  ];
  const productDesignImageDir = imageDirCandidates.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = productDesignImageDir
    ? fs
        .readdirSync(productDesignImageDir, { withFileTypes: true })
  ...
  return pdfEntries
    .map((f) => {
      ...
      if (matchedImage && productDesignImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(productDesignImageDir), matchedImage);
      } else if (remainingImages.length && productDesignImageDir) {
        imageHref = toStudyImageHref(path.basename(productDesignImageDir), remainingImages.shift()!);
      ...
      const meta = productDesignMeta[f.name];
      const title = isReadableText(meta?.title) ? meta!.title : prettifyName(f.name);
      const description = isReadableText(meta?.description)
        ? meta!.description
        : 'Product design study material (PDF).';

      return {
        title,
        description,
        href: `/product-design-books/${encodeURIComponent(f.name)}`,
        imageHref,
      };
    })
...
export default function StudyingProductDesignItemsPage() {
  const items = getProductDesignBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">
          Product design study books
        </h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/product-design</strong> or{' '}
          <strong className="font-semibold text-ink">studying/Product design</strong>. Cover images:{' '}
          <strong className="font-semibold text-ink">image/Product design</strong>. Metadata:{' '}
          <strong className="font-semibold text-ink">productDesignStudyBookMetadata.json</strong> — run{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run product-design-books:metadata
          </code>
          .
        </p>
        ...
        {items.length === 0 ? (
          <p className="mt-8 rounded-xl border border-parchment-300/80 bg-parchment-50/90 px-4 py-6 font-sans text-sm leading-relaxed text-ink-muted">
            ...
            <code className="font-mono text-ink">studying/product-design</code> or{' '}
            <code className="font-mono text-ink">studying/Product design</code>, then run{' '}
            <code className="font-mono text-ink">npm run product-design-books:metadata</code>.
          </p>
        ) : (
