...
import businessFinanceStudyBookMetadata from '@/data/businessFinanceStudyBookMetadata.json';
import { resolveBusinessFinanceStudyBookDirs } from '@/lib/businessFinanceStudyDirs';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type BusinessFinanceBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const businessFinanceMeta = businessFinanceStudyBookMetadata as Record<string, BusinessFinanceBookMeta>;
...
function getBusinessFinanceBookItems(): StudyItem[] {
  const bookDirs = resolveBusinessFinanceStudyBookDirs();
  if (!bookDirs.length) return [];

  const imageDirCandidates = [
    path.join(process.cwd(), 'image', 'Business & finance'),
    path.join(process.cwd(), 'image', 'business-finance'),
    path.join(process.cwd(), 'image', 'business finance'),
    path.join(process.cwd(), 'image', 'Business and finance'),
  ];
  const businessFinanceImageDir = imageDirCandidates.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = businessFinanceImageDir
    ? fs
        .readdirSync(businessFinanceImageDir, { withFileTypes: true })
  ...
  return pdfEntries
    .map((f) => {
      ...
      if (matchedImage && businessFinanceImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(businessFinanceImageDir), matchedImage);
      } else if (remainingImages.length && businessFinanceImageDir) {
        imageHref = toStudyImageHref(path.basename(businessFinanceImageDir), remainingImages.shift()!);
      ...
      const meta = businessFinanceMeta[f.name];
      const title = isReadableText(meta?.title) ? meta!.title : prettifyName(f.name);
      const description = isReadableText(meta?.description)
        ? meta!.description
        : 'Business and finance study material (PDF).';

      return {
        title,
        description,
        href: `/business-finance-books/${encodeURIComponent(f.name)}`,
        imageHref,
      };
    })
...
export default function StudyingBusinessFinanceItemsPage() {
  const items = getBusinessFinanceBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">
          Business and finance study books
        </h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/business-finance</strong> or{' '}
          <strong className="font-semibold text-ink">studying/Business & finance</strong>. Cover images:{' '}
          <strong className="font-semibold text-ink">image/Business & finance</strong>. Metadata:{' '}
          <strong className="font-semibold text-ink">businessFinanceStudyBookMetadata.json</strong> — run{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run business-finance-books:metadata
          </code>
          .
        </p>
        ...
        {items.length === 0 ? (
          <p className="mt-8 rounded-xl border border-parchment-300/80 bg-parchment-50/90 px-4 py-6 font-sans text-sm leading-relaxed text-ink-muted">
            ...
            <code className="font-mono text-ink">studying/business-finance</code> or{' '}
            <code className="font-mono text-ink">studying/Business & finance</code>, then run{' '}
            <code className="font-mono text-ink">npm run business-finance-books:metadata</code>.
          </p>
        ) : (
