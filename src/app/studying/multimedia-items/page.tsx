...
import multimediaStudyBookMetadata from '@/data/multimediaStudyBookMetadata.json';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type MultimediaBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const multimediaMeta = multimediaStudyBookMetadata as Record<string, MultimediaBookMeta>;
...
function getMultimediaBookItems(): StudyItem[] {
  const booksDir = path.join(process.cwd(), 'studying', 'multimedia');
  if (!fs.existsSync(booksDir)) return [];

  const multimediaImageDirs = [
    path.join(process.cwd(), 'image', 'multimedia'),
    path.join(process.cwd(), 'image', 'Multimedia'),
  ];
  const multimediaImageDir = multimediaImageDirs.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = multimediaImageDir
    ? fs
        .readdirSync(multimediaImageDir, { withFileTypes: true })
  ...
  return files
    ...
    .map((f) => {
      ...
      if (matchedImage && multimediaImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(multimediaImageDir), matchedImage);
      } else if (remainingImages.length && multimediaImageDir) {
        imageHref = toStudyImageHref(path.basename(multimediaImageDir), remainingImages.shift()!);
...
export default function StudyingMultimediaItemsPage() {
  const items = getMultimediaBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">
          Multimedia study books
        </h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/multimedia</strong>. Cover images can live in{' '}
          <strong className="font-semibold text-ink">image/multimedia</strong> (match the PDF basename when possible).
          Titles and summaries are stored in{' '}
          <strong className="font-semibold text-ink">multimediaStudyBookMetadata.json</strong> — refresh with{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run multimedia-books:metadata
          </code>{' '}
