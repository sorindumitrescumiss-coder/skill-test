/**
 * Reads pages 1–5 of PDFs in studying/business-finance (and aliases) and writes
 * src/data/businessFinanceStudyBookMetadata.json.
 * Run: node scripts/extract-business-finance-book-metadata.mjs
 */
import fs from 'fs';
...
const BUSINESS_FINANCE_FOLDER_NAMES = ['business-finance', 'Business & finance', 'business finance'];
const outPath = path.join(root, 'src', 'data', 'businessFinanceStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const businessBoost =
    /\b(business|finance|financial|accounting|bookkeeping|enterprise|erp|budget|forecast|invoicing|payments|expense|management|capital|cash\s*flow|investment|economics|entrepreneur|trade\s+finance|working\s+capital|balance\s+sheet|income\s+statement|business\s+planning)\b/i;
  ...
  for (const line of head) {
    ...
    if (businessBoost.test(line)) score += 38;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && businessBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Business and finance study material (PDF).';
...
async function main() {
  ...
  const bookDirs = BUSINESS_FINANCE_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'business-finance');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Business and finance study material (PDF).' };
      }
    }
  }
