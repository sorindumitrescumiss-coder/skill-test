/**
 * Reads pages 1–5 of each PDF in studying/multimedia and writes
 * src/data/multimediaStudyBookMetadata.json with inferred title + summary text.
 * Run: node scripts/extract-multimedia-book-metadata.mjs
 */
import fs from 'fs';
...
const booksDir = path.join(root, 'studying', 'multimedia');
const outPath = path.join(root, 'src', 'data', 'multimediaStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const multimediaBoost =
    /\b(multimedia|interactive\s+media|video|film|cinema|television|broadcast|streaming|podcast|audio\s+production|sound\s+design|music\s+production|\bDAW\b|mixing|mastering|recording|voice\s+over|motion\s+graphics|animation|\bVFX\b|visual\s+effects|compositing|color\s+grading|editing|\bNLE\b|premiere|after\s+effects|davinci|final\s+cut|photogrammetry|\bAR\b|\bVR\b|compression|codec|subtitle|captioning|digital\s+signage)\b/i;
  ...
  for (const line of head) {
    ...
    if (multimediaBoost.test(line)) score += 38;
    if (/introduction|handbook|manual|guide|edition|course|workshop|tutorial|basics|fundamentals|primer|essential/i.test(line))
      score += 14;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && multimediaBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Multimedia study material (PDF).';
...
async function main() {
  ...
  for (const name of files) {
    ...
    try {
      ...
      out[name] = { title: prettify(name), description: 'Multimedia study material (PDF).' };
    }
  }
