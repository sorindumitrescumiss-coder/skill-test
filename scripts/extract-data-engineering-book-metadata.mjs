/**
 * Reads pages 1–5 of PDFs in studying/data-engineering (and aliases) and writes
 * src/data/dataEngineeringStudyBookMetadata.json.
 * Run: node scripts/extract-data-engineering-book-metadata.mjs
 */
import fs from 'fs';
...
const DATA_ENGINEERING_FOLDER_NAMES = ['data-engineering', 'Data engineering', 'data engineering', 'data enginnering'];
const outPath = path.join(root, 'src', 'data', 'dataEngineeringStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const dataBoost =
    /\b(cybersecurity|cyber\s+security|information\s+security|infosec|penetration\s+testing|ethical\s+hacking|vulnerability|threat|malware|ransomware|phishing|\bSIEM\b|\bSOC\b|incident\s+response|digital\s+forensics|cryptograph|encryption|zero\s+trust|firewall|\bIDS\b|\bIPS\b|OWASP|NIST|compliance|risk\s+management|network\s+security|cloud\s+security|application\s+security|identity\s+and\s+access|\bIAM\b)\b/i;
  ...
  for (const line of head) {
    ...
    if (dataBoost.test(line)) score += 38;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && dataBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'Data engineering study material (PDF).';
...
async function main() {
  ...
  const bookDirs = DATA_ENGINEERING_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'data-engineering');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'Data engineering study material (PDF).' };
      }
    }
  }
