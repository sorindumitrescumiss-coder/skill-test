...
import dataEngineeringStudyBookMetadata from '@/data/dataEngineeringStudyBookMetadata.json';
import { resolveDataEngineeringStudyBookDirs } from '@/lib/dataEngineeringStudyDirs';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type DataEngineeringBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const dataEngineeringMeta = dataEngineeringStudyBookMetadata as Record<string, DataEngineeringBookMeta>;
...
function getDataEngineeringBookItems(): StudyItem[] {
  const bookDirs = resolveDataEngineeringStudyBookDirs();
  if (!bookDirs.length) return [];

  const imageDirCandidates = [
    path.join(process.cwd(), 'image', 'Data engineering'),
    path.join(process.cwd(), 'image', 'Data engineering'),
    path.join(process.cwd(), 'image', 'Data engineering'),
    path.join(process.cwd(), 'image', 'Data engineering'),
  ];
  const dataEngineeringImageDir = imageDirCandidates.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = dataEngineeringImageDir
    ? fs
        .readdirSync(dataEngineeringImageDir, { withFileTypes: true })
  ...
  return pdfEntries
    .map((f) => {
      ...
      if (matchedImage && dataEngineeringImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(dataEngineeringImageDir), matchedImage);
      } else if (remainingImages.length && dataEngineeringImageDir) {
        imageHref = toStudyImageHref(path.basename(dataEngineeringImageDir), remainingImages.shift()!);
      ...
      const meta = dataEngineeringMeta[f.name];
      const title = isReadableText(meta?.title) ? meta!.title : prettifyName(f.name);
      const description = isReadableText(meta?.description)
        ? meta!.description
        : 'Data engineering study material (PDF).';

      return {
        title,
        description,
        href: `/data-engineering-books/${encodeURIComponent(f.name)}`,
        imageHref,
      };
    })
...
export default function StudyingDataEngineeringItemsPage() {
  const items = getDataEngineeringBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">
          Data engineering study books
        </h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/data-engineering</strong> or{' '}
          <strong className="font-semibold text-ink">studying/data enginnering</strong>. Cover images:{' '}
          <strong className="font-semibold text-ink">image/Data engineering</strong>. Metadata:{' '}
          <strong className="font-semibold text-ink">dataEngineeringStudyBookMetadata.json</strong> — run{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run data-engineering-books:metadata
          </code>
          .
        </p>
        ...
        {items.length === 0 ? (
          <p className="mt-8 rounded-xl border border-parchment-300/80 bg-parchment-50/90 px-4 py-6 font-sans text-sm leading-relaxed text-ink-muted">
            ...
            <code className="font-mono text-ink">studying/data-engineering</code> or{' '}
            <code className="font-mono text-ink">studying/data enginnering</code>, then run{' '}
            <code className="font-mono text-ink">npm run data-engineering-books:metadata</code>.
          </p>
        ) : (
