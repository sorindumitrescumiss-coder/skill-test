...
import devopsCloudStudyBookMetadata from '@/data/devopsCloudStudyBookMetadata.json';
import { resolveDevopsCloudStudyBookDirs } from '@/lib/devopsCloudStudyDirs';

export const dynamic = 'force-dynamic';

type StudyItem = { title: string; description: string; href: string; imageHref?: string | null };
type DevopsBookMeta = { title: string; description: string };
const IMAGE_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.webp', '.gif'];
const devopsMeta = devopsCloudStudyBookMetadata as Record<string, DevopsBookMeta>;
...
function getDevopsCloudBookItems(): StudyItem[] {
  const bookDirs = resolveDevopsCloudStudyBookDirs();
  if (!bookDirs.length) return [];

  const imageDirCandidates = [
    path.join(process.cwd(), 'image', 'DevOps  cloud'),
    path.join(process.cwd(), 'image', 'DevOps cloud'),
    path.join(process.cwd(), 'image', 'devopsCloud'),
    path.join(process.cwd(), 'image', 'devops-cloud'),
    path.join(process.cwd(), 'image', 'devops cloud'),
  ];
  const devopsImageDir = imageDirCandidates.find((dir) => fs.existsSync(dir));
  ...
  const imageFiles = devopsImageDir
    ? fs
        .readdirSync(devopsImageDir, { withFileTypes: true })
  ...
  return pdfEntries
    .map((f) => {
      ...
      if (matchedImage && devopsImageDir) {
        const idx = remainingImages.indexOf(matchedImage);
        if (idx >= 0) remainingImages.splice(idx, 1);
        imageHref = toStudyImageHref(path.basename(devopsImageDir), matchedImage);
      } else if (remainingImages.length && devopsImageDir) {
        imageHref = toStudyImageHref(path.basename(devopsImageDir), remainingImages.shift()!);
...
export default function StudyingDevopsCloudItemsPage() {
  const items = getDevopsCloudBookItems();
    ...
    <AppLayout activePath="/studying">
      <div className="mx-auto max-w-6xl px-4 pb-16 pt-10 sm:px-6 md:pt-14">
        ...
        <h1 className="mt-2 font-serif text-3xl font-semibold tracking-tight text-ink md:text-[2.05rem]">
          DevOps / cloud study books
        </h1>
        <p className="mt-3 font-sans text-[15px] leading-relaxed text-ink-muted">
          PDFs load from <strong className="font-semibold text-ink">studying/devops-cloud</strong> or your{' '}
          <strong className="font-semibold text-ink">studying/DevOps  cloud</strong> folder. Cover images:{' '}
          <strong className="font-semibold text-ink">image/DevOps  cloud</strong> or{' '}
          <strong className="font-semibold text-ink">image/devops-cloud</strong>. Metadata:{' '}
          <strong className="font-semibold text-ink">devopsCloudStudyBookMetadata.json</strong> — run{' '}
          <code className="rounded bg-parchment-200/80 px-1.5 py-0.5 font-mono text-[13px] text-ink">
            npm run devops-cloud-books:metadata
          </code>
          .
        </p>
        ...
        {items.length === 0 ? (
          <p className="mt-8 rounded-xl border border-parchment-300/80 bg-parchment-50/90 px-4 py-6 font-sans text-sm leading-relaxed text-ink-muted">
            ...
            <code className="font-mono text-ink">studying/devops-cloud</code> or{' '}
            <code className="font-mono text-ink">studying/DevOps  cloud</code>, then run{' '}
            <code className="font-mono text-ink">npm run devops-cloud-books:metadata</code>.
          </p>
        ) : (
