/**
 * Reads pages 1–5 of PDFs in studying/devops-cloud (and aliases) and writes
 * src/data/devopsCloudStudyBookMetadata.json.
 * Run: node scripts/extract-devops-cloud-book-metadata.mjs
 */
import fs from 'fs';
...
const DEVOPS_CLOUD_FOLDER_NAMES = [
  'devops-cloud',
  'devops cloud',
  'DevOps  cloud',
  'DevOps cloud',
...
const outPath = path.join(root, 'src', 'data', 'devopsCloudStudyBookMetadata.json');
...
function chooseTitle(lines, fileName) {
  ...
  const devopsBoost =
    /\b(DevOps|\bCI\s*\/\s*CD\b|continuous\s+(integration|delivery|deployment)|Kubernetes|\bK8s\b|Docker|container|orchestration|Terraform|Pulumi|Ansible|Chef|Puppet|infrastructure\s+as\s+code|\bIaC\b|GitOps|cloud\s+computing|\bAWS\b|\bAzure\b|\bGCP\b|Google\s+Cloud|observability|monitoring|Prometheus|Grafana|logging|ELK|microservices|release\s+engineering|site\s+reliability|\bSRE\b|platform\s+engineering)\b/i;
  ...
  for (const line of head) {
    ...
    if (devopsBoost.test(line)) score += 38;
    if (/introduction|handbook|manual|guide|edition|course|tutorial|fundamentals|best\s+practices|patterns/i.test(line))
      score += 12;
    ...
    if (letters > 8 && upper / letters > 0.5 && line.length < 110 && devopsBoost.test(line)) score += 24;
    if (score > bestScore) {
      bestScore = score;
      best = line;
    }
...
function chooseSummary(lines, title) {
  ...
  if (!summary) return 'DevOps / cloud study material (PDF).';
...
async function main() {
  ...
  const bookDirs = DEVOPS_CLOUD_FOLDER_NAMES.map((n) => path.join(studying, n)).filter((dir) => fs.existsSync(dir));
  const primaryDir = path.join(studying, 'devops-cloud');
  ...
  for (const booksDir of bookDirs.length ? bookDirs : [primaryDir]) {
    ...
    for (const name of names) {
      ...
      try {
        ...
        out[name] = { title: prettify(name), description: 'DevOps / cloud study material (PDF).' };
      }
    }
  }
